...
const encrypt = (letter, value) => {
    const index = alphabets.indexOf(letter);
    if (index === -1) return letter; // Keep non-alphabet characters unchanged
    const shift = (index + (value % alphabets.length)) % alphabets.length; // Use modulo to handle large shifts
    return alphabets[shift];
};
...
const decrypt = (letter, value) => {
    const index = alphabets.indexOf(letter);
    if (index === -1) return letter; // Keep non-alphabet characters unchanged
    const shift = (index - (value % alphabets.length) + alphabets.length) % alphabets.length; // Use modulo to handle large shifts
    return alphabets[shift];
};
